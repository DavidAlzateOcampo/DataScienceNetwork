...
#Add a survived variable
test.survived<-data.frame(Survived=rep("None",nrow(test)),test[,])
#train.reduced<-train

#combine data sets
#data.combined<-rbind(train[,!("PassengerId")],test.survived)
train$Survived<-as.factor(train$Survived)
data.combined<-full_join(train,test.survived)
data.combined$Pclass<-as.factor(data.combined$Pclass)
table(data.combined$Survived)

p<-ggplot(train,aes(x=Pclass,fill=Survived))
p<- p + geom_bar()+xlab("Pclass")
p<-p+ylab("Total Count")+labs(fill="Survived")
p
...
p<-ggplot(data.combined[1:891,],aes(x=titles,fill=Survived))
p<-p+geom_histogram(binwidth = 0.5,stat="count") + facet_wrap(~Pclass)
p<-p+ggtitle("Pclass")+xlab("Title")+ylab("Total Count")
p<-p+labs(fill="Survived")
p
...
p<-ggplot(data.combined[1:891,],aes(x=Sex,fill=Survived))
p<-p+geom_histogram(binwidth = 0.5,stat="count") + facet_wrap(~Pclass)
p<-p+ggtitle("Pclass")+xlab("Title")+ylab("Total Count")
p<-p+labs(fill="Survived")
p
